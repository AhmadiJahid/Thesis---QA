...
def main():
    ...
    config = {
        "seed": args.seed,
        "model_id": args.model_id or base_config.get("model_id") or "mistralai/Mistral-7B-Instruct-v0.1",
        "model_name": base_config.get("model_name", "mistral_7b_instruct"),
    ...
    # 3. Load Data
    # Path is: components/router/models/mistral_7b_instruct/router.py
    ...
    if config["device"] == "cuda":
        ...
        mem_alloc = torch.cuda.get_device_properties(0).total_memory / (1024**3)
        print(f"GPU: {dev} (total VRAM: {mem_alloc:.1f} GB)")
